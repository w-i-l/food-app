        ...
        NavigationView {
            ZStack{
                ...
                ScrollView{
                    ...
                    NavigationLink(destination:Search(itemsInCart: $itemsInCart).navigationBarHidden(true)) {
                        ZStack{
                    ...
                    VStack{
                        ...
                        ForEach(Array(stride(from: 0, to: restaurantsArray.count, by: 2)),id:\.self){num in
                            ...
                            HStack(spacing:25){
                                ...
                                NavigationLink(destination:Restaurant(itemsInCart:$itemsInCart,name: first.name, logo: first.logo, stars: first.stars, distance: first.distance, noOfCoupons: first.noOfcoupons, items: first.items).navigationBarHidden(true)){
                                    RestaurantCard(image: first.image, logo: first.logo)}
                                
                                if num+1<restaurantsArray.count{
                                    let second = restaurantsArray[num+1]
                                    NavigationLink(destination:Restaurant(itemsInCart:$itemsInCart,name: second.name, logo: second.logo, stars: second.stars, distance: second.distance, noOfCoupons: second.noOfcoupons, items: second.items).navigationBarHidden(true)){
...
struct Home_Preview:PreviewProvider{
    static var previews: some View{
        ...
        NavigationView {
            ZStack {
                Home(itemsInCart: FoodApp().$itemsInCart)
